...
class CsvGrid(ttk.Frame):
    ...
    def __init__(self, master, on_dirty, on_ctrl_t=None, **kw):
        super().__init__(master, **kw)
        self.on_dirty  = on_dirty
        self.on_ctrl_t = on_ctrl_t   # optional app-level handler for Ctrl+T
    ...
    _BATCH_SIZE = 40   # rows built per idle slice
    ...
    def _rebuild(self):
        ...
        for w in self._hdr_inner.winfo_children():
            w.destroy()
        self._cells = []
        self._rebuild_token = getattr(self, "_rebuild_token", 0) + 1
        ...
        # ── Data rows — built in batches to keep the UI responsive ───────────
        self._build_batch(0, self._rebuild_token)

    def _build_batch(self, start: int, token: int):
        if token != self._rebuild_token:
            return   # a newer _rebuild() superseded this one
        end = min(start + self._BATCH_SIZE, len(self.rows))
        for ri in range(start, end):
            self._cells.append(self._build_row_widgets(ri, self.rows[ri]))
        if end < len(self.rows):
            self.after(0, lambda: self._build_batch(end, token))
        else:
            self._update_scrollregion()
...
class AltiumCSVLibrarian(tk.Tk):
    ...
    def __init__(self):
        ...
        self._dirty_ui_job           = None   # debounce handle for visual dirty updates
    ...
    def _build_ui(self):
        ...
        tk.Label(
            ...
            padx=0, pady=0,
            anchor="s",
        ).pack(side=LEFT, anchor="s", pady=14)
    ...
    def _paint_banner(self, photo):
        ...
        # Measure approximate title width to position version beside it
        canvas.create_text(
            236, 46,
            text=VERSION,
            font=("Arial", 8),
    ...
    def _show_help(self):
        ...
        COMMANDS = [
            ...
            ("Editing", [
                ...
                ("Ctrl + T",        "capacitors.csv: normalise columns & build Description"),
                ("Ctrl + Q",        "Autosize all columns to fit content"),
            ]),
            ("File", [
                ("Ctrl + S",        "Save ALL modified files to disk"),
                ("💾  Save button",  "Save active file only (backup written first)"),
    ...
    # ── Capacitor auto-describe (Ctrl+D) ─────────────────────────────────────

    # Order matches the desired output: Library Ref-Value-Voltage-Dielectric
    _CAP_SRC_COLS = ("Library Ref", "Value", "Voltage", "Dielectric")
    _CAP_DST_COL  = "Description"

    @staticmethod
    def _normalize_cap_value(col: str, raw: str) -> str:
        """Return a cleaned value for a capacitor source column.

        Dielectric / Voltage  → all uppercase, spaces removed
        Value                 → all lowercase then 'f' uppercased, spaces removed
        """
        v = str(raw or "").replace(" ", "")
        c = col.lower()
        if c in ("dielectric", "voltage"):
            return v.upper()
        if c == "value":
            v = v.lower()
            v = v.replace("f", "F")
    ...
    def _auto_describe(self) -> bool:
        """Normalise source columns then build Description for capacitors.csv.

        Returns True if handled (active file is capacitors.csv), False so the
        grid falls back to its copy-from-above behaviour for any other file.
        """
        if not self._active or self._active.lower() != "capacitors.csv":
            return False

        headers = self._grid.headers
        rows    = self._grid.rows

        required = (*self._CAP_SRC_COLS, self._CAP_DST_COL)
        missing  = [c for c in required if c not in headers]
        if missing:
            messagebox.showwarning(
                "Auto-describe",
                f"Columns not found in {self._active}:\n" + ", ".join(missing),
            )
            return True

        dst_ci      = headers.index(self._CAP_DST_COL)
        src_indices = {c: headers.index(c) for c in self._CAP_SRC_COLS}
        any_change  = False

        for ri, row in enumerate(rows):
            # ── 1. Normalise each source column in-place ──────────────────────
            for col in self._CAP_SRC_COLS:
                raw        = str(row.get(col, "") or "")
                normalised = self._normalize_cap_value(col, raw)
                if normalised != raw:
                    row[col] = normalised
                    self._grid._cells[ri][src_indices[col]]._var.set(normalised)
                    any_change = True

            # ── 2. Build description (join non-empty parts, no spaces) ────────
            parts = [str(row.get(c, "") or "") for c in self._CAP_SRC_COLS]
            desc  = "-".join(p for p in parts if p)
            row[self._CAP_DST_COL] = desc
            self._grid._cells[ri][dst_ci]._var.set(desc)
            any_change = True

        if any_change:
            self._grid._mark_dirty()
            self._update_status("Descriptions built & columns normalised", "saved")
        else:
            self._update_status("Already up to date", "muted")
    ...
    def _on_grid_dirty(self):
        if not self._active:
            return
        # Mark dirty and reschedule autosave immediately (cheap)
        self._files[self._active]["dirty"] = True
        self._schedule_autosave()
        # Debounce the expensive visual updates — only run 150 ms after typing stops
        if self._dirty_ui_job:
            self.after_cancel(self._dirty_ui_job)
        self._dirty_ui_job = self.after(150, self._flush_dirty_ui)

    def _flush_dirty_ui(self):
        self._dirty_ui_job = None
        if not self._active:
            return
        self._update_row_count()
        self._refresh_file_list()
    ...
    def _schedule_autosave(self):
        ...
        # Only start the tick loop if it isn't already running
        if not self._autosave_tick_job:
            self._tick_autosave()
    ...
    def _save_file(self, name: str, silent: bool = False):
        ...
        if not f:
            return
        # Pull live data from the grid if this is the active file
        if name == self._active:
            f["headers"], f["rows"] = self._grid.get_data()
        path: Path = f["path"]
        try:
            make_backup(path)
