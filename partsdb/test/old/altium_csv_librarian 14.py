...
class CsvGrid(ttk.Frame):
    ...
    def __init__(self, master, on_dirty, **kw):
        ...
        # Horizontal scroll moves BOTH the header frame and the data canvas
    ...
    def _xscroll_both(self, *args):
        """Scroll both the data canvas and the header canvas horizontally."""
    ...
    def _on_inner_configure(self, event):
        self._canvas.configure(scrollregion=self._canvas.bbox("all"))
        # Keep header canvas width in sync with data canvas content width
        try:
            self._header_canvas.configure(
                scrollregion=self._header_canvas.bbox("all")
            )
    ...
    def _on_canvas_configure(self, event):
        ...
        # Sync header scroll position whenever canvas is resized
        try:
