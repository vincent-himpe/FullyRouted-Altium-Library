...
class CsvGrid(ttk.Frame):
    ...
    def add_row(self, source_row_index: int | None = None):
        """Append (or insert after source_row_index) a new row with a GUID."""
        new_row: dict = {}
        for i, h in enumerate(self.headers):
            if i == 0:
                new_row[h] = new_guid()
            elif source_row_index is not None and source_row_index < len(self.rows):
                new_row[h] = self.rows[source_row_index].get(h, "")
            else:
                new_row[h] = ""

        if source_row_index is not None and source_row_index < len(self.rows):
            self.rows.insert(source_row_index + 1, new_row)
        else:
        ...
        insert_ri = (source_row_index + 1) if source_row_index is not None else len(self.rows) - 1
        self._focus_cell(insert_ri, 1)
    ...
    def _rebuild(self):
        ...
        col_widths = [GUID_COL_W] + [max(MIN_COL_W, len(h) * 8 + 16) for h in self.headers[1:]]

        # ── Header row (row 0) ────────────────────────────────────────────────
        header_cells = []
        for ci, h in enumerate(self.headers):
            bg = C["header_bg"]
            fg = C["text_muted"] if ci == 0 else C["text_muted"]
            lbl = tk.Label(
                self._inner, text=h.upper(), bg=bg, fg=fg,
                font=("Consolas", 9, "bold"), anchor=W,
                padx=8, pady=6, relief=FLAT,
                width=col_widths[ci] // 8,
            )
            lbl.grid(row=0, column=ci + 1, sticky="nsew", padx=(0, 1), pady=(0, 1))
            header_cells.append(lbl)
        # Row-number header
        tk.Label(
            self._inner, text="#", bg=C["header_bg"], fg=C["text_dim"],
            font=("Consolas", 9), width=4, pady=6,
        ).grid(row=0, column=0, sticky="nsew", padx=(0, 1), pady=(0, 1))
        self._cells.append(header_cells)
        ...
        for ri, row in enumerate(self.rows):
            ...
            rn.grid(row=ri + 1, column=0, sticky="nsew", padx=(0, 1), pady=(0, 1))
            ...
            for ci, h in enumerate(self.headers):
                ...
                entry.grid(row=ri + 1, column=ci + 1, sticky="nsew", padx=(0, 1), pady=(0, 1))
                entry._row = ri
                entry._col = ci
                entry._var = var

                if not is_guid:
                    var.trace_add("write", lambda *_, r=ri, c=ci, v=var, h=h: self._on_edit(r, h, v))
    ...
    def _focus_cell(self, row_i: int, col_i: int):
        ...
        # col_i is the column index in self.headers; skip 0 (GUID)
        col_i = max(1, col_i)
        grid_row = row_i + 1   # +1 because row 0 is headers
        if grid_row < len(self._cells) and col_i < len(self._cells[grid_row]):
            w = self._cells[grid_row][col_i]
    ...
    def _on_keypress(self, event, row_i: int, col_i: int):
        ...
        # ── Ctrl+Down — insert cloned row below ──────────────────────────────
        if ctrl and key == "Down":
            self.add_row(source_row_index=row_i)
            return "break"
...
class AltiumCSVLibrarian(tk.Tk):
    ...
    def _open_file(self, name: str):
        ...
        self._active = name
        f = self._files[name]
        self._grid.load(f["headers"], f["rows"])
        self._show_grid()
        self._file_title_lbl.config(text=name)
        self._update_row_count()
        self._refresh_file_list()
        self._update_status_from_dirty(f["dirty"])
    ...
    def _add_row(self):
        ...
        if not self._active:
            return
        self._grid.add_row(source_row_index=None)
